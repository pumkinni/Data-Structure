// Practice2
// 주어진 BST 에서 노드 간의 차이값 중 최소 값을 구하세요.

// 입력 트리: 4, 2, 6, 1, 3
// 출력: 1

// 입력 트리: 5, 1, 48, null, null, 12, 51
// 출력: 3
// levelOrder  방식을 사용하여 queue에서 데이터를 꺼내 자식 노드를 찾아 넣으며 차이값을 구하기

import java.util.ArrayList;
import java.util.Collections;
import java.util.LinkedList;
import java.util.Queue;

public class Practice2 {

    public static void solution(Integer[] data) {
        BinarySearchTree bst = new BinarySearchTree();
        ArrayList<Integer> list = new ArrayList<>();

        for (int i = 0; i < data.length; i++) {
            if (data[i] == null){
                continue;
            }
            bst.addNode(data[i]);
        }

        // levelOrder
        Queue<Node> queue = new LinkedList();
        queue.add(bst.head);
        while (!queue.isEmpty()){
            Node cur = queue.poll();
            if (cur.left != null){
                queue.offer(cur.left);
                list.add((int)(cur.key - cur.left.key));
            }
            if (cur.right != null){
                queue.offer(cur.right);
                list.add((int)(cur.right.key - cur.key));
            }
        }
        // 최솟값 구하기
        // 방법 1
//        int min = Integer.MAX_VALUE;
//        for (int item: list) {
//            min = Math.min(min, item);
//        }
        // 방법 2
//        min = list.stream().min((x1, x2) -> x1 > x2 ? 1 : -1).get();

        // 방법 3
        int min = Collections.min(list);
        System.out.println(min);

    }

    public static void main(String[] args) {
        // Test code
        Integer[] data = {3, 1, 4, null, 2};
        solution(data);

        data = new Integer[]{5, 1, 48, null, null, 12, 51};
        solution(data);
    }
}
